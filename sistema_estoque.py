...
def carregar_log():
    try:
        ...
        return df
    except: return pd.DataFrame(columns=["Data", "Produto", "Quantidade", "Tipo", "Detalhe", "Usuario"])

def registrar_log(produto, quantidade, tipo, origem_destino, usuario="Sistema"):
    try:
        novo = pd.DataFrame([{"Data": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Produto": produto, "Quantidade": quantidade, "Tipo": tipo, "Detalhe": origem_destino, "Usuario": usuario}])
...
def criar_pdf_unificado(lista_carga):
    ...
    try:
        ...
        pdf.cell(190, 10, txt=f"Data: {datetime.now().strftime('%d/%m/%Y')}", ln=True, align='C'); pdf.ln(10)
...
if tela == "Estoque":
    ...
    with c2:
        ...
        if st.session_state['carga_acumulada']:
            ...
            except: st.dataframe(dfc, use_container_width=True)
