...
# --- CONFIGURAÇÃO ---
st.set_page_config(page_title="Sistema Gestão 36.0 (Compras)", layout="wide", initial_sidebar_state="collapsed")
...
def init_state():
    ...
    if 'last_forn' not in st.session_state: st.session_state['last_forn'] = "Todos"
...
def criar_pdf_pedido(dataframe, fornecedor, total):
    try:
        ...
        for i, r in dataframe.iterrows():
            prod = str(r['Produto'])[:45].encode('latin-1', 'replace').decode('latin-1')
            padr = str(r['Padrao']).encode('latin-1', 'replace').decode('latin-1')
            
            pdf.cell(90, 8, prod, 1, 0, 'L')
            pdf.cell(30, 8, padr, 1, 0, 'C')
            pdf.cell(20, 8, str(int(r['🛒 Comprar'])), 1, 0, 'C')
            pdf.cell(25, 8, f"R$ {r['Custo']:.2f}", 1, 0, 'R')
            pdf.cell(25, 8, f"R$ {r['Total Item']:.2f}", 1, 1, 'R')
        
        pdf.ln(5)
        pdf.set_font("Arial", 'B', 12)
        pdf.cell(190, 10, txt=f"VALOR TOTAL DO PEDIDO: R$ {total:,.2f}", ln=True, align='R')
...
if tela == "Compras":
    ...
    with col_esq:
        ...
        lista_fornecedores = ["Todos"] + sorted([str(x) for x in df_db['Fornecedor'].unique() if str(x) != 'nan'])
        forn_sel = st.selectbox("🏢 Filtrar por Fornecedor:", lista_fornecedores)
        
        # Reseta cache se mudar filtro
        if forn_sel != st.session_state['last_forn']:
    ...
    with col_dir:
        ...
        st.write("")
        # 2. Botão Mágico
        if st.button("🪄 Calcular Sugestão (Meta - Estoque Total)", use_container_width=True):
            # Filtra dados
            if forn_sel == "Todos": df_calc = df_db.copy()
            else: df_calc = df_db[df_db['Fornecedor'] == forn_sel].copy()
            
            # Lógica: Meta Global = Min_SA + Min_SI
            # Estoque Total = Central + SA + SI
            # Sugestão = Meta - Estoque Total
            ...
            df_calc['🛒 Comprar'] = (df_calc['Meta Global'] - df_calc['Estoque Total']).apply(lambda x: max(0, int(x)))
    ...
    if st.session_state['compras_df_cache'] is not None:
        ...
        df_view['🛒 Comprar'] = 0 # Inicia zerado

    # Filtro de Busca
    busca = st.text_input("🔍 Buscar Produto:", "")
    if busca: df_view = df_view[df_view['Produto'].str.contains(busca, case=False, na=False)]
    
    # Calcula Valor Visualmente
    df_view['Valor Total'] = df_view['🛒 Comprar'] * df_view['Custo']

    # Editor
    edited_df = st.data_editor(
        df_view[['Produto', 'Fornecedor', 'Padrao', 'Estoque Total', 'Meta Global', 'Custo', '🛒 Comprar']],
        column_config={
            ...
            "Custo": st.column_config.NumberColumn("R$ Unit.", disabled=True, format="%.2f"),
            "🛒 Comprar": st.column_config.NumberColumn(min_value=0, step=1, format="%d")
        },
    ...
    # Rodapé
    total_itens = int(edited_df['🛒 Comprar'].sum())
    total_valor = (edited_df['🛒 Comprar'] * edited_df['Custo']).sum()
    
    st.divider()
    m1, m2, m3 = st.columns(3)
    m1.metric("Itens no Pedido", total_itens)
    m2.metric("Valor Total Estimado", f"R$ {total_valor:,.2f}")
    
    # Botão Gerar
    if m3.button("📄 Gerar Pedido (PDF/Excel)", type="primary"):
        itens_ped = edited_df[edited_df['🛒 Comprar'] > 0].copy()
        itens_ped['Total Item'] = itens_ped['🛒 Comprar'] * itens_ped['Custo']
        
        if itens_ped.empty:
            st.warning("Nenhum item para comprar.")
        else:
            # Gera PDF
            pdf_bytes = criar_pdf_pedido(itens_ped, forn_sel, total_valor)
            st.session_state['pedido_pdf'] = pdf_bytes
            
            # Gera Excel
            buf = io.BytesIO()
            with pd.ExcelWriter(buf, engine='openpyxl') as writer:
                itens_ped.to_excel(writer, index=False, sheet_name='Pedido')
            st.session_state['pedido_xlsx'] = buf.getvalue()
            
            registrar_log("Vários", total_itens, "Pedido Compra", f"Forn: {forn_sel} | R$ {total_valor:.2f}")
            st.rerun()
    ...
    if st.session_state['pedido_pdf']:
        ...
        d1, d2 = st.columns(2)
        d1.download_button("⬇️ Baixar PDF", st.session_state['pedido_pdf'], f"Pedido_{forn_sel}.pdf", "application/pdf")
        d2.download_button("⬇️ Baixar Excel", st.session_state['pedido_xlsx'], f"Pedido_{forn_sel}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")


# =================================================================================
# 🚚 TELA DE TRANSFERÊNCIA (MANTIDA IGUAL)
# =================================================================================
