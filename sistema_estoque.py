...
st.set_page_config(page_title="Sistema Estável 27.1", layout="wide")
...
def calcular_cmv_mensal():
    if not os.path.exists(ARQUIVO_LOG) or not os.path.exists(ARQUIVO_DADOS): return pd.DataFrame()
...
def renderizar_baixa_por_arquivo(df_geral, loja):
    st.markdown("---")
    with st.expander("📉 Baixar Vendas (Arquivo)", expanded=True):
        f = st.file_uploader("Relatório", type=['csv', 'xlsx'], key="up_ven")
        if f:
            try:
                ...
                cn = c1.selectbox("Coluna Produto", cols, index=in_n)
                cq = c2.selectbox("Coluna Qtd", cols, index=in_q)
                ...
                if st.button("🚀 Processar"):
                    ...
                    for i, r in df.iterrows():
                        ...
                        if q > 0:
                            m = (df_geral['Loja']==loja) & (df_geral['Produto']==p)
                            if m.any():
                                cur = df_geral.loc[m, 'Estoque_Atual'].values[0]
                                df_geral.loc[m, 'Estoque_Atual'] = max(0, cur-q)
                                suc+=1
                            else: err.append(f"{p} ({q})")
                    if suc>0:
                        salvar_dados(df_geral); registrar_log("Lote", suc, "Venda", loja)
                        st.success(f"Baixados: {suc}")
                        if err: st.write("Não encontrados:", err)
                        st.rerun()
            except: st.error("Erro no arquivo")

# --- INTERFACE ---
st.title("🚀 Sistema Integrado 27.1")
...
with st.sidebar:
    ...
    if modo in ["Estoque Central", "Hosp. Santo Amaro", "Hosp. Santa Izabel"]:
        with st.expander("📦 Upload Estoque"):
            f = st.file_uploader("Arquivo", type=['csv', 'xlsx'], key="upe")
            if f:
                try:
                    df = pd.read_csv(f) if f.name.endswith('.csv') else pd.read_excel(f)
                    mapa = {}
    ...
    with st.expander("💲 Preços"):
        f = st.file_uploader("Arquivo", type=['csv', 'xlsx'], key="upp")
...
if modo == "📊 Dashboard":
    ...
    if st.session_state['distribuicao_concluida']:
        ...
        # Usamos .copy() para garantir que não haja referências cruzadas
        # E evitamos inplace=True para não ter risco de NoneType
        ed = st.data_editor(
            df_view[cols], 
            column_config={
                "Estoque_Atual": st.column_config.NumberColumn(disabled=True),
                "Saldo": st.column_config.NumberColumn(disabled=True),
                "Tem SA": st.column_config.NumberColumn(disabled=True),
                "Med SA": st.column_config.NumberColumn(disabled=True),
                "Tem SI": st.column_config.NumberColumn(disabled=True),
                "Med SI": st.column_config.NumberColumn(disabled=True)
            }, use_container_width=True, height=500
        )
        
        if not ed.equals(df_view[cols]):
            # Atualização SEGURA sem inplace
            ed_idx = ed.set_index('Produto')
            w_idx = df_w.set_index('Produto')
            w_idx.update(ed_idx[['Env SA', 'Env SI']])
            st.session_state['df_distribuicao_temp'] = w_idx.reset_index()
            st.rerun()
    ...
    if not df_l.empty:
        # AQUI É A BAIXA POR ARQUIVO QUE VOCÊ PEDIU
