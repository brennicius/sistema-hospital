...
# --- FUNÇÕES DE LIMPEZA (NOVA INTELIGÊNCIA) ---
def limpar_numero(valor):
    """Limpa sujeira de números (R$, kg, un, espaços) e corrige vírgula"""
    if pd.isna(valor): return 0.0
    if isinstance(valor, (int, float)): return float(valor)
    
    # Remove textos comuns
    s = str(valor).lower().replace('r$', '').replace('kg', '').replace('un', '').replace('lt', '').replace(' ', '')
    
    # Corrige pontuação (1.000,00 -> 1000.00)
...
with st.sidebar:
    ...
    # UPLOAD INTELIGENTE DE ESTOQUE (CORREÇÃO AQUI)
            ...
            if f:
                try:
                    ...
                    for i, row in df_temp.head(20).iterrows():
                        ...
                        if any("prod" in x or "cod" in x or "item" in x for x in s): 
                            header_row = i
                            break
                    ...
                    # 2. AUTO-MAPA INTELIGENTE
                    mapa = {}
                    for c in df.columns:
                        cl = str(c).lower()
                        if "prod" in cl or "nome" in cl or "item" in cl: mapa[c] = "Produto"
                        elif any(x in cl for x in ['est', 'qtd', 'quant', 'saldo', 'contagem', 'total', 'físico']): mapa[c] = "Estoque_Atual"
                    ...
                    if "Produto" in df.columns:
                        if "Estoque_Atual" in df.columns:
                            # APLICA A LIMPEZA NOS NÚMEROS AQUI!
                        ...
                        resetar_processos(); st.success(f"Estoque de {modo} atualizado com sucesso!"); st.rerun()
                    else:
                        st.error(f"Não achei a coluna 'Produto'. Colunas encontradas: {list(df.columns)}")
                except Exception as e: st.error(f"Erro: {e}")
